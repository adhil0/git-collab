...
ggplot(pal_pika, aes(x=longitude, y=latitude) ) +
  geom_bin2d(bins = 70) +
...
#Set your API Key
ggmap::register_google(key = "AIzaSyDYBTCIRVKRUvuGnyxTnajMonSNYBcER8A")
p <- ggmap(get_googlemap(center = c(lon = -105.335167, lat = 35),
                    zoom = 11, scale = 2,
                    maptype ='terrain',
                    color = 'color'))
col1 = "#011f4b"
p + geom_point(aes(x = longitude, y = latitude), colour = col1, data = pal_pika, alpha=0.25, size = 0.5) + 
